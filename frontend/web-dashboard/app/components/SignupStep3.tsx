...
import { Briefcase, Clock, Calendar, ChevronDown, ChevronUp, CheckCircle, Mail } from 'lucide-react';
...
const SignupStep3: React.FC<SignupStep3Props> = ({ onNext, isLoading = false, userEmail = '' }) => {
    const [showConfirmation, setShowConfirmation] = useState(false);
    const [registrationData, setRegistrationData] = useState<SignupStep3Schema | null>(null);
    ...
    const [openDropdown, setOpenDropdown] = useState<'workType' | 'shiftType' | 'currentJobStatus' | null>(null);
    ...
    const toggleWorkTypeSelection = (value: SignupStep3Schema['workType'][number]) => {
        const newValues = workType.includes(value) ? workType.filter((v) => v !== value) : [...workType, value];
        setValue('workType', newValues, { shouldValidate: true });
    };
    
    const toggleShiftTypeSelection = (value: SignupStep3Schema['shiftType'][number]) => {
        const newValues = shiftType.includes(value) ? shiftType.filter((v) => v !== value) : [...shiftType, value];
                        ...
                        {userEmail && (
                            <>
                                <div className="bg-gray-50 rounded-lg p-3 mb-4">
                                    <p className="text-gray-900 font-medium text-sm sm:text-base break-all">{userEmail}</p>
                                </div>
        ...
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-auto my-4 ">
            <div className="p-6 sm:p-8">
                <div className="text-center mb-6">
                    ...
                    <p className="text-gray-600 text-sm sm:text-base">Step 3 of 3 - Work Preferences</p>
                </div>
                ...
                <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-6">
                        ...
                        <div className="relative">
                            ...
                            >
                                <div className="flex items-center">
                                    ...
                                    <span className={workType.length === 0 ? 'text-gray-400' : ''}>
                                        {workType.length === 0
                                            ? 'Select work types'
                                            : workType.map(w => w.replace(/([A-Z])/g, ' $1').trim()).join(', ')}
                                    </span>
                                </div>
                            ...
                            {openDropdown === 'workType' && (
                                <div className="absolute z-10 mt-1 w-full bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                                    {(['FullTime', 'PartTime', 'PerDiem', 'LiveIn'] as const).map((type) => (
                                        <div
                                            key={type}
                        ...
                        <div className="relative">
                            ...
                            >
                                <div className="flex items-center">
                                    ...
                                    <span className={shiftType.length === 0 ? 'text-gray-400' : ''}>
                                        {shiftType.length === 0 ? 'Select shift types' : shiftType.join(', ')}
                                    </span>
                                </div>
                            ...
                            {openDropdown === 'shiftType' && (
                                <div className="absolute z-10 mt-1 w-full bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                                    {(['Day', 'Night', 'Weekend', 'Overnight', 'Flexible'] as const).map((type) => (
                                        <div
                                            key={type}
                            ...
                            {errors.shiftType && <p className="mt-1 text-sm text-red-600">{errors.shiftType.message}</p>}
                        </div>
                    </div>
                    ...
                    <div ref={currentJobStatusRef}>
                        ...
                        <div className="relative">
                                ...
                                <div className="absolute z-10 mt-1 w-full bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                                    {(
                                        ...
                                        <div
                                            ...
                                            onClick={() => {
                                                setValue('currentJobStatus', status, { shouldValidate: true });
                                                setOpenDropdown(null);
                                            }}
                                        >
                                            {status
                            ...
                            {errors.currentJobStatus && <p className="mt-1 text-sm text-red-600">{errors.currentJobStatus.message}</p>}
                        </div>
                    </div>
                ...
                <div className="mt-6 text-center">
                    <p className={`text-gray-600 text-sm ${isLoading ? 'opacity-50' : ''}`}>
