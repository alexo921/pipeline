...
const ForgotPasswordPage: React.FC = () => {
  ...
  const onSubmit = async (values: ForgotPasswordSchema) => {
    ...
    try {
      ...
      console.log('Reset password email sent to:', values.email.trim());
      setIsSubmitted(true);
    } catch (error) {
          ...
          <>
            ...
            <div className="mt-6 text-center">
              <p className="text-gray-500 text-xs leading-relaxed">
                For your security, we'll only send reset instructions to email addresses associated with an active account.
              </p>
            </div>
          </>
        ) : (
          <>
            <div className="text-center mb-8">
              ...
              <div className="bg-gray-50 rounded-lg p-3 mb-6">
                <p className="text-gray-900 font-medium text-sm sm:text-base break-all">{getValues('email')}</p>
              </div>
